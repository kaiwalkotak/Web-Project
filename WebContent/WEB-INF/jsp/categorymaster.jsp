                            ...
                            <div class="col-sm-12 col-lg-12">
                               <div class="block full">
                                    ...
                                    <div class="block-title">
                                        <h2><i class="fa fa-tasks"></i><strong> Category </strong> List</h2>
		     	...
		     	<div class="modal-content">
				    <!-- Add Contact Block -->
                                <div class="block" style="margin-bottom: 0px;padding-bottom: 0px;">
                                    ...
                                    <!-- Add Contact Content -->
                                    <form:form id="addCategoryForm" modelAttribute="addCategory" method="POST" class="form-horizontal form-bordered">
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label" for="categoryname">Category Name:</label>
                                            <div class="col-xs-9">
...
$('#example-datatable').dataTable({
  ...
  function createNewCategory(id){
		...
		if(id!="0"){
		 $.ajax({
	   			type:"POST",
	   			url:"${pageContext.servletContext.contextPath}/masters/getcategorydetails",
	   			data:$("#addCategoryForm").serialize()+"&mainCatId="+id.split("_")[1],
	   			dataType: 'json',
	   			async:false,
	   			success: function(response){
	   				$("#categoryname").focus().val(response.categoryname);
  ...
  function deleteCategory(id){
		...
		$.ajax({
			type:"GET",
			url:"${pageContext.servletContext.contextPath}/masters/deleteCategory",
			...
			success: function(response){
				...
				if(response == "false"){
					alertDiv("alert-danger","This Category has some data in the system, so you can not delete this category!!");
				}else{
					$("#top-loading-stop").click();
					$("#mainCatTr_"+id.split("_")[1]).remove();
					alertDiv("alert-success","Category deleted successfully!!");
				}
			}
		});
  ...
  function saveCategory(){
		...
		$('#addCategoryForm').validate().element("#categoryname");
		if($("#categoryname").val() != ""){
			$("#top-loading-start").click();
			$.ajax({
				type:"POST",
				url:"${pageContext.servletContext.contextPath}/masters/addCategory",
				data:$("#addCategoryForm").serialize(),
				dataType: 'json',
				async:false,
				success: function(response){
				if(response.split("~")[0] == "y"){
						var msgText  = $("#category_id").val() == 0 ? "redirect_success_common" : "redirect_success_common_update";
						$("#top-loading-stop").click();			
						window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/categorymaster/W+AWN0BPnNGXrFrq9xQpUQ==/1/"+msgText+"/0";
					}else{
