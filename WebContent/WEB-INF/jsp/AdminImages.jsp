...
<body>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1>
								<i class="gi gi-nameplate_alt sidebar-nav-icon"></i>Admin Images
							</h1>
						</div>
									...
									<c:forEach var="tableDetails" items="${dataTableList}"
										...
										<tr id="mainPrdTr_${tableDetails[0]}">
											...
											<c:forEach begin="0" end="${fn:length(tableDetails)-1}"
												...
												<c:if test="${dataIndex.index ne 0 && dataIndex.index ne 3 && dataIndex.index eq 2}">
													<td><a
												...
												<c:if
													test="${dataIndex.index ne 0 && dataIndex.index ne 3 && dataIndex.index ne 2}">
													<td class="text-center">${tableDetails[dataIndex.index]}</td>
												</c:if>

												<c:if
													test="${dataIndex.index eq 3  && dataIndex.index ne 0 && dataIndex.index ne 2}">
					...
					<div class="block full subclass" style="display: none;">
						...
						<div class="block-title">
							...
							<h2>
								<strong>Add </strong>Images Details
							</h2>
						</div>
						<!-- Form  -->
	...
	<script>
		...
		function createNewBanner(id) {
			...
			if (id != "0") {
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/getImageAdmin",
		...
		function saveBanner() {
			...
			if (document.getElementById("title").value != '') {
				...
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/addImagesAdmin",
							data : $("#addBannerForm").serialize(),
							dataType : 'json',
							async : false,
							success : function(response) {
								if (response.split("~")[0] == "y") {
									...
									if ('${isInnerCall}' == "1") {
										...
										window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/AdminImages/KdBYQUXvG1uhcbCrWzYLrw==/1/"
												+ msgText + "/0";
		...
		function deleteProduct(id) {
			...
			if (confirm("Press OK to Delete this Image From Karma Website !")) {
				$("#top-loading-start").click();
				$
						.ajax({
							type : "GET",
							url : "${pageContext.servletContext.contextPath}/masters/deleteAdminImage",
			...
			.on(
					...
					function() {
						...
						$
								.ajax({
									type : "GET",
									url : "${pageContext.request.contextPath}/masters/setAdminImageType",
