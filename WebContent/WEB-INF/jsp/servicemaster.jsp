                            ...
                            <div class="col-sm-12 col-lg-12">
                               <div class="block full">
                                    ...
                                    <div class="block-title">
                                        <h2><i class="fa fa-tasks"></i><strong> Services </strong> List</h2>
		     	...
		     	<div class="modal-content">
				    <!-- Add Contact Block -->
                                <div class="block" style="margin-bottom: 0px;padding-bottom: 0px;">
                                    <!-- Add Contact Title -->
                                    <div class="block-title">
                                        <h2><i class="fa fa-plus"></i> Add New Service</h2>
                                    </div>
                                    <!-- END Add Contact Title -->

                                    <!-- Add Contact Content -->
                                    <form:form id="addServiceForm" modelAttribute="addService" method="POST" class="form-horizontal form-bordered">
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label" for="servicecode">Service Code :</label>
                                            <div class="col-xs-9">
                                                <input type="text" name="servicecode" id="servicecode" class="form-control" placeholder="Enter service code">
                                            </div>
                                        </div>
                                         <div class="form-group">
                                            <label class="col-xs-3 control-label" for="notes">Description :</label>
                                            <div class="col-xs-9">
                                                <textarea id="notes" name="notes" rows="3" class="form-control" placeholder="Description.."></textarea>
                                            </div>
                                        </div>
										<div class="form-group form-actions">
                                            <div class="col-xs-9 col-xs-offset-3" style="text-align: right;">
                                            	<input type="hidden" value="0" id="service_id" name="service_id">
...
$('#example-datatable').dataTable({
  ...
  function createNewCategory(id){
		...
		if(id!="0"){
		 $.ajax({
	   			type:"POST",
	   			url:"${pageContext.servletContext.contextPath}/masters/getservicedetails",
	   			data:$("#addServiceForm").serialize()+"&mainCatId="+id.split("_")[1],
	   			dataType: 'json',
	   			async:false,
	   			success: function(response){
	   				$("#servicecode").focus().val(response.servicecode);
  ...
  function deleteCategory(id){
		...
		$.ajax({
			type:"GET",
			url:"${pageContext.servletContext.contextPath}/masters/deleteServicetype",
			...
			success: function(response){
				...
				if(response == "false"){
					alertDiv("alert-danger","This service has some data in the system, so you can not delete this service!!");
				}else{
					$("#top-loading-stop").click();
					$("#mainCatTr_"+id.split("_")[1]).remove();
					alertDiv("alert-success","Service deleted successfully!!");
				}
			}
		});
  ...
  function saveCategory(){
		...
		$('#addServiceForm').validate().element("#servicecode");
		if($("#servicecode").val() != ""){
			$("#top-loading-start").click();
			$.ajax({
				type:"POST",
				url:"${pageContext.servletContext.contextPath}/masters/addServices",
				data:$("#addServiceForm").serialize(),
				dataType: 'json',
				async:false,
				success: function(response){
				if(response.split("~")[0] == "y"){
						var msgText  = $("#service_id").val() == 0 ? "redirect_success_common" : "redirect_success_common_update";
						$("#top-loading-stop").click();			
						window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/servicemaster/DuH06GS6QVHUbsTX7G4U8g==/1/"+msgText+"/0";
					}else{
