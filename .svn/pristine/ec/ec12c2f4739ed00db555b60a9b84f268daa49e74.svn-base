...
<body>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1>
								<i class="gi gi-comments sidebar-nav-icon"></i>Payment Login
							</h1>
						</div>
					...
					<div class="block full mainclass">
						...
						<div class="block-title">
							<h2>
								<strong>Admin </strong>payment Login
							</h2>
									...
									<c:forEach var="tableDetails" items="${dataTableList}"
										...
										<tr id="mainPrdTr_${tableDetails[0]}">
											...
											<c:forEach begin="0" end="${fn:length(tableDetails)}"
												...
												<c:if
													test="${dataIndex.index eq 6  && dataIndex.index ne 0}">
											...
											<td align="center">
												...
												</button> <%-- <button type="button" id="prd1_${tableDetails[0]}"
					...
					<div class="block full subclass" style="display: none;">
						...
						<form:form id="addBannerForm" modelAttribute="addBannerForm"
							...
							<div class="form-group">
								<div class="col-md-6">
									<label for="example-nf-password">Username</label> <input
										type="text" id="username" name="username" class="form-control"
										placeholder="username">
								</div>
								<div class="col-md-6">
									<label for="example-nf-password">Email - Id</label> <input
							...
							<div class="form-group">
								<div class="col-md-6">
									<label for="example-nf-password">Passwrod</label> <input
								...
								<div class="col-md-6">
									<label for="example-nf-password">Phone Number</label> <input
										type="text" id="phoneno" name="phoneno" class="form-control"
	...
	<script>
		...
		function createNewBanner(id) {
			...
			if (id != "0") {
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/getPaymentDetials",
		...
		function saveBanner() {
			...
			$('#addBannerForm').validate().element("#username");
			$('#addBannerForm').validate().element("#email");
			$('#addBannerForm').validate().element("#phoneno");
			$('#addBannerForm').validate().element("#password");
			...
			if (document.getElementById("username").value != ''
				...
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/paymentlogin",
							data : $("#addBannerForm").serialize(),
							dataType : 'json',
							async : false,
							success : function(response) {
								if (response.split("~")[0] == "y") {
									...
									if ('${isInnerCall}' == "1") {
										...
										window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/paymentlogin/qBfGJE3bYMqHYuRiKw9R3w==/1/"
												+ msgText + "/0";
		...
		function deleteProduct(id) {
			...
			if (confirm("Press OK to Delete this EMI Detail!")) {
				...
				$
						.ajax({
							type : "GET",
							url : "${pageContext.servletContext.contextPath}/masters/",
		...
		$('#addBannerForm').validate(
				{
					...
					messages : {
						...
						phoneno : 'Please enter PhoneNumber!',
				...
				.on(
						...
						function() {
							...
							$
									.ajax({
										type : "GET",
										url : "${pageContext.request.contextPath}/masters/setPaymentType",
