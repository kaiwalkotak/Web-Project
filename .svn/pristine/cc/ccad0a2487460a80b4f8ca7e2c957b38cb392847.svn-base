                    ...
                    <div id="page-content">
                        ...
                        <div class="content-header">
                            <div class="header-section">
                                <h1>
                                    <i class="fa fa-tasks"></i>Car Type
                                </h1>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-12 col-lg-12">
                               <div class="block full">
                                    ...
                                    <div class="block-title">
                                        <h2><i class="fa fa-tasks"></i><strong> Car Type</strong> List</h2>
		     	...
		     	<div class="modal-content">
				    <!-- Add Contact Block -->
                                <div class="block" style="margin-bottom: 0px;padding-bottom: 0px;">
                                    <!-- Add Contact Title -->
                                    <div class="block-title">
                                        <h2><i class="fa fa-plus"></i> Add New Car type</h2>
                                    </div>
                                    <!-- END Add Contact Title -->

                                    <!-- Add Contact Content -->
                                    <form:form id="addCartypeForm" modelAttribute="addCartype" method="POST" class="form-horizontal form-bordered">
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label" for="cartypename">Car Type :</label>
                                            <div class="col-xs-9">
                                                <input type="text" name="cartypename" id="cartypename" class="form-control" placeholder="Enter Car Type">
                                            </div>
                                        </div>
                                         <div class="form-group">
                                                <label class="col-xs-3 control-label" for="add-contact-name">Fuel Type :</label>
                                                <div class="col-xs-9">
                                                    <select id="fueltype" name="fueltype" class="form-control" data-placeholder="Select Fuel Type" style="width: 250px;">
                                                        <c:forEach var="fueltype" items="${fueltypeList}">
                                                        	<option value="${fueltype[1]}" >${fueltype[1]}</option>
                                                        </c:forEach>
                                                    </select>
                                                </div>
                                            </div>
										<div class="form-group form-actions">
                                            <div class="col-xs-9 col-xs-offset-3" style="text-align: right;">
                                            	<input type="hidden" value="0" id="cartype_id" name="cartype_id">
...
$('#example-datatable').dataTable({
  ...
  function createNewCategory(id){
		...
		if(id!="0"){
		 $.ajax({
	   			type:"POST",
	   			url:"${pageContext.servletContext.contextPath}/masters/getCartypedetails",
	   			data:$("#addCartypeForm").serialize()+"&mainCatId="+id.split("_")[1],
	   			dataType: 'json',
	   			async:false,
	   			success: function(response){
	   				$("#cartypename").focus().val(response.cartypename);
  ...
  function deleteCategory(id){
		...
		$.ajax({
			type:"GET",
			url:"${pageContext.servletContext.contextPath}/masters/deleteCartype",
			...
			success: function(response){
				...
				if(response == "false"){
					alertDiv("alert-danger","This car type has some cars in system, so you can not delete this cartype!!");
				}else{
					$("#top-loading-stop").click();
					$("#mainCatTr_"+id.split("_")[1]).remove();
					alertDiv("alert-success","Car type deleted successfully!!");
				}
			}
		});
  ...
  function saveCategory(){
		...
		$('#addCartypeForm').validate().element("#cartypename");
		if($("#cartypename").val() != ""){
			$("#top-loading-start").click();
			$.ajax({
				type:"POST",
				url:"${pageContext.servletContext.contextPath}/masters/addCartype",
				data:$("#addCartypeForm").serialize(),
				dataType: 'json',
				async:false,
				success: function(response){
				if(response.split("~")[0] == "y"){
						var msgText  = $("#cartype_id").val() == 0 ? "redirect_success_common" : "redirect_success_common_update";
						$("#top-loading-stop").click();			
						window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/carstype/T3DKmGIm0ZUQz3DmLOfv1A==/1/"+msgText+"/0";
					}else{
  ...
  $('#addCartypeForm').validate({
	    ...
	    messages: {
	    	cat_name: 'Please enter car type !'
	    }
	});
