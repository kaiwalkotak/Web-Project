                    ...
                    <div id="page-content">
                       ...
                       <!-- Customer Content -->
                        <div class="row">
                                  ...
                                  <!-- Interactive Block -->
                                    <div class="block">
                                        ...
                                        <div class="block-title">
                                            ...
                                            <h2 style="zoom: 0.85; padding-bottom: 1.5%;"><i class="fa fa-user"></i> ${md.preMemberName} ${md.memberName} | Util. Nights: ${utiNights}| Bal. Nights: ${remNights} | Pend. Amt : ${pendingamaount} | <span class="label label-danger">Prin.Realized :  <fmt:formatNumber type="number" value="${alldet[4]}" pattern="0.00" maxFractionDigits="2"/> %</span> </h2>
                                        </div>
                           ...
                           <div id="alertDivAjax" class="alert " role="alert" style="display:none;">
						       ${successMsg}
						    </div>
                           <form:form id="interaction-form">
                           <%-- <input type="hidden" value="${md.id}" name="memberId" id="memberId"/>
                           <input type="hidden" value="${md.membercode}" name="memberCd" id="memberCd"/> --%>
                        </form:form>
                       </div>
                    </div>
                    <!-- END Page Content -->
...
<script src="${pageContext.request.contextPath}/resources/js/commonValidation.js"></script>
        ...
        function saveticket(ac){
        	if(validateData('.required-validate','required') == false){
        		$('#page-wrapper').addClass('page-loading');
				$("#savebtn_"+ac).text('Please Wait ...');
				setTimeout(function () {
        		$.ajax({
            		type:"POST",
            		url:"${pageContext.servletContext.contextPath}/ticket/saveticket",
            		data:$("#interaction-form").serialize()+"&savecnt="+ac+"&subtype="+$("#intertypesubid_"+ac+" option:selected").text()+"&onlytype="+$("#intertypeid_"+ac+" option:selected").text()+"&inteonly="+$("#interid_"+ac+" option:selected").text()+"&empname="+$("#employeeId_"+ac+" option:selected").text()+"&memberId="+"${md.id}"+"&memberCd="+"${md.membercode}",
            		dataType: 'json',
            		async:false,
            		success: function(response){
            			if(response.split("~")[0] == "y"){
            				$("#isnewticket_"+ac).val("old_"+response.split("~")[2]);
            				$("#tickno_"+ac).html("<strong>Ticket No : </strong> "+response.split("~")[3]);
            				alertDivAjax("alert-success","Your New ticket has been generated !!");
            			}else{
            				alertDivAjax("alert-danger","Something went wrong, Please try again later !!");
            			}
            		}
            	});
        		$('#page-wrapper').removeClass('page-loading');
				$("#savebtn_"+ac).text('SAVE');
				},10);
        	}
        }
        var ticketcnt = 1;
        function addnewticket(newticketcnt){
        	var innerHtml = '<div class="block">'
	            		  ...
	            		  + '<a href="javascript:void(0)" class="btn btn-alt btn-sm btn-primary" data-toggle="block-toggle-fullscreen"><i class="fa fa-desktop"></i></a>'
	            		  + '</div>'
	            		  + '<input type="hidden" value="0" name="tat_'+ticketcnt+'" id="tat_'+ticketcnt+'" ><h2 id="tickno_'+ticketcnt+'"><strong>Ticket No : </strong> </h2>'
	            		  + '<input type="hidden" value="'+(newticketcnt == "-1" ? "1" : "0")+'" name="isfromadmin_'+ticketcnt+'" id="isfromadmin_'+ticketcnt+'" ></div>'
	            		  + '<div class="block-content">'
	            		  + '<div class="row">'
	            		  + '<div class="form-group">'
	            		  + '<label class="col-md-2 control-label" for="example-text-input" style="margin-top: 5px;">Interaction:</label>'
	            		  + '<div class="col-md-3">'
	            		  + '<select id="interid_'+ticketcnt+'" name="interid_'+ticketcnt+'" class="form-control required-validate" size="1" onchange="changeinte(0,this.id)" data-toggle="tooltip" data-placement="top" title="Interaction">'
                         ...
                         + '</select>'
                         + '</div>'
                         + '<label class="col-md-3 control-label" for="example-text-input" style="margin-top: 5px;">Ticket From :</label>'
                         + '<div class="col-md-3">'
                         + '<select id="ticketfrom_'+ticketcnt+'" name="ticketfrom_'+ticketcnt+'" class="form-control required-validate" size="1" data-toggle="tooltip" data-placement="top" title="Interaction">'
                         ...
                         + '<div class="col-md-3">'
                         + '<select id="intertypeid_'+ticketcnt+'" name="intertypeid_'+ticketcnt+'" class="form-control required-validate" size="1" onchange="changeinte(1,this.id)" data-toggle="tooltip" data-placement="top" title="Interaction type">'
                         + '<option value="">-- Select Interaction --</option>'
                         + '</select>'
                         + '</div>'
                         + '<label class="col-md-3 control-label" for="example-text-input" style="margin-top: 5px;">Interaction Sub Type:</label>'
                         + '<div class="col-md-3">'
                         + '<select id="intertypesubid_'+ticketcnt+'" name="intertypesubid_'+ticketcnt+'" onchange="setactualdate(this.id,'+ticketcnt+')" class="form-control required-validate" size="1" data-toggle="tooltip" data-placement="top" title="Interaction sub type">'
                         + '<option value="">-- Select Interaction --</option>'
                         + '</select>'
                         + '</div>'
                         + '</div>'
                         ...
                         + '<div class="col-md-3">'
                         + '<input type="text" id="approxresolveddate_'+ticketcnt+'" name="approxresolveddate_'+ticketcnt+'" class="form-control" placeholder="Expected close Date" autocomplete="off" readonly="readonly">'
                         + '</div>'
                         + '<label class="col-md-3 control-label" for="example-text-input" style="margin-top: 5px;">Actual close date:</label>'
                         + '<div class="col-md-3">'
                         + '<input type="text" id="actualresolveddate_'+ticketcnt+'" name="actualresolveddate_'+ticketcnt+'" class="form-control" placeholder="Actual close Date" autocomplete="off" readonly="readonly">'
                         + '</div>'
                         + '</div>'
                         + '</div>'
                         + '<div class="row" style="margin-top:1%;">'
                         ...
                         + '<div class="col-md-9">'
                         + '<textarea id="membercomment_'+ticketcnt+'" name="membercomment_'+ticketcnt+'" rows="4" class="form-control required-validate" cols=""></textarea>'
                         + '</div>'
                         + '</div>'
                         + '<div class="row" style="margin-top:1%;">'
                         + '<div class="form-group">'
                         ...
                         + '<div class="form-group">'
                         + '<button type="button" id="savebtn_'+ticketcnt+'" class="btn btn-sm btn-info" onclick="saveticket('+ticketcnt+');">SAVE</button>'
                         + '</div>'
                         + '<p class="text-muted" style="margin-bottom:4%">&nbsp;</p>'
                         + '</div>'
                         + '<ul id="commentdiv_'+ticketcnt+'" class="media-list push" style="margin-top: 10%;">'
                         + '</ul>'
                         + '<ul class="media-list push" style="margin-top: 2%;">'
                         + '<li class="media">'
                         + '<a href="javascript:void(0)" class="pull-left">'
                         + '<img src="${pageContext.servletContext.contextPath}/resources/img/placeholders/avatars/avatar.jpg" alt="Avatar" class="img-circle">'
                         + '</a>'
                         + '<div class="media-body">'
                         + '<textarea id="commentarea_'+ticketcnt+'" name="commentarea_'+ticketcnt+'" class="form-control" rows="2" placeholder="Your comment.."></textarea>'
                         + '<button type="button" class="btn btn-xs btn-primary" onclick="addnewcomment('+ticketcnt+');" style="float:right;margin-top:2%;"><i class="fa fa-pencil"></i> Post Comment</button>'
                         + '</div>'
                         + '</li>'
                         + '</ul>'
                         + '</div>'
        ...
        function getexistingticket(){
        	...
        	$.ajax({
        		type:"POST",
        		url:"${pageContext.servletContext.contextPath}/ticket/getallticket",
        		data:$("#interaction-form").serialize()+"&memberMainId="+"${md.id}",
        		dataType: 'json',
        		async:false,
        		success: function(response){
        			var curcnt = 0;
        				for(var i=0;i<response.length;i++){
        					curcnt = addnewticket(response[i].id);
        					$("#tat_"+curcnt).val(response[i].tat);$("#isfromadmin_"+curcnt).val(response[i].isadminentry);$("#interid_"+curcnt).val(response[i].interactionsubtype.interactiontype.interaction.id);
        					changeinte("0","interid_"+curcnt);
        					$("#intertypeid_"+curcnt).val(response[i].interactionsubtype.interactiontype.id);
        					changeinte("1","intertypeid_"+curcnt);
        					$("#intertypesubid_"+curcnt).val(response[i].interactionsubtype.id);$("#ticketfrom_"+curcnt).val(response[i].ticketfrom);
        					$("#approxresolveddate_"+curcnt).val(moment(response[i].approxresolveddate).format('DD/MM/YYYY'));$("#actualresolveddate_"+curcnt).val(response[i].actualresolveddate == null ? '' : moment(response[i].actualresolveddate).format('DD/MM/YYYY'));
        					$("#employeeId_"+curcnt).val(response[i].employeeId == 0 ? "" : response[i].employeeId);$("#membercomment_"+curcnt).val(response[i].membercomment);$("#usercomment_"+curcnt).val(response[i].usercomment);$("#status_"+curcnt).val(response[i].status);
        					$("#tickno_"+curcnt).html("<strong>Ticket No : </strong> "+response[i].ticketno);
        					getallcomments(response[i].id,curcnt);
        				}
        		}
        	});
        	$('#page-wrapper').removeClass('page-loading');
        ...
        function getallcomments(tickid,curcnt){
        	$.ajax({
        		type:"POST",
        		url:"${pageContext.servletContext.contextPath}/ticket/getallticketcomment",
        		data:$("#interaction-form").serialize()+"&tickid="+tickid,
        		dataType: 'json',
        		async:false,
        		success: function(response){
        				for(var i=0;i<response.length;i++){
        					displaycomment(response[i],curcnt);
        				}
        		}
        	});
        }
        function setactualdate(sid,tcnt){
        	var tat = getaTatdata($("#"+sid).val());
        	$("#tat_"+tcnt).val(tat);
        	var nextmonthdate = addBusinessDays(moment(),parseInt(tat));
        	$("#approxresolveddate_"+tcnt).val(moment(nextmonthdate).format('DD/MM/YYYY'));
        }
        function addnewcomment(curid){
        	var cm = $("#commentarea_"+curid).val();
        	if(cm.trim() != ""){
        		if(($("#isnewticket_"+curid).val()).split("_")[0] == "old"){
        			$.ajax({
                		type:"POST",
                		url:"${pageContext.servletContext.contextPath}/ticket/saveticketcomment",
                		data:$("#interaction-form").serialize()+"&ticketId="+($("#isnewticket_"+curid).val()).split("_")[1]+"&textcomment="+cm+"&isfromuser=0",
                		dataType: 'json',
                		async:false,
                		success: function(response){
                			if(response.comment != "SAVE_ERROR"){
                				displaycomment(response,curid);
                			}
                			
                		}
                	});
        		}else{
        			alert("Please save ticket !!");
        		}
        	}else{
        		alert("Please enter comments !!");
        	}
        }
        function displaycomment(response,curid){
        	var innerHTML = '<li class="media">'
                + '<a href="javascript:void(0);" class="pull-left">'
                + '<img src="${pageContext.servletContext.contextPath}/resources/img/placeholders/avatars/avatar15.jpg" alt="Avatar" class="img-circle">'
                + '</a>'
                + '<div class="media-body">'
                + '<a href="javascript:void(0);"><strong>'+response.username+'</strong></a>'
                + '<span class="text-muted" style="float:right;"><small><em>Date : '+(moment(response.createddatetime).format('DD/MM/YYYY hh:mm A'))+'</em></small></span>'
                + '<p>'+response.comment+'</p>'
                + '</div>'
                + '</li>';
        		$("#commentdiv_"+curid).append(innerHTML);
        		$("#commentarea_"+curid).val('');
        }
        ...
        function getaTatdata(id){
        	var tat = "";
        	<c:forEach var="tatList" items="${tatList}" >
        		if(id == "${tatList[0]}"){
        			tat = "${tatList[1]}";
        		}
        	</c:forEach>
