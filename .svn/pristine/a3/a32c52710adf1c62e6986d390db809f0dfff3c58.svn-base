...
<body>
				...
				<div id="page-content">
					...
					<div class="content-header">
						<div class="header-section">
							<h1>
								<i class="gi gi-comments sidebar-nav-icon"></i>Admin Campaign Master
							</h1>
						</div>
					...
					<ul class="breadcrumb breadcrumb-top">
						<li>Dashboard</li>
						<li><a href="">Campaign Master</a></li>
					</ul>
					...
					<div class="block full mainclass">
						...
						<div class="block-title">
							<h2>
								<strong>Admin </strong>Campaign Master
							</h2>
					...
					<div class="block full subclass" style="display: none;">
						...
						<div class="block-title">
							...
							<h2>
								<strong>Add </strong>Campaign
							</h2>
						</div>
						<form:form id="addBannerForm" modelAttribute="addBannerForm">
							<div class="form-group">
								<div class="col-md-12">
									<label for="example-nf-password">Campaign ID</label> <input
										type="text" id="c_id" name="c_id" class="form-control"
										placeholder="Campaign Id"> <input type="hidden"
										id="bnr_id" name="bnr_id" value="0">
								</div>
							</div>
							<div class="form-group">
								<div class="col-md-12">
									<label for="example-nf-password">Campaign Name</label> <input
	...
	<script>
		...
		function createNewBanner(id) {
			...
			if (id != "0") {
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/getCampaign",
		...
		function saveBanner() {
			...
			$('#addBannerForm').validate().element("#c_id");
			...
			if (document.getElementById("name").value != '') {
				...
				$
						.ajax({
							type : "POST",
							url : "${pageContext.servletContext.contextPath}/masters/addCampaign",
							data : $("#addBannerForm").serialize(),
							dataType : 'json',
							async : false,
							success : function(response) {
								if (response.split("~")[0] == "y") {
									...
									if ('${isInnerCall}' == "1") {
										...
										window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/CampaignMaster/tqpx6K0n8j20ThGlcVv5kA==/1/"
												+ msgText + "/0";
				...
				if (saveDataMsg.split("~")[0] == "y") {
					...
					window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/adminTestimonial/0Nvmp2YMIWvYvXRJce6vOg==/1/"
							+ msgText + "/0";
				} else {
		...
		function deleteProduct(id) {
			...
			if (confirm("Press OK to Delete this Campaign!")) {
				$("#top-loading-start").click();

				$
						.ajax({
							type : "GET",
							url : "${pageContext.servletContext.contextPath}/masters/deleteCampaign",
							...
							success : function(response) {
								...
								if (response == false) {
									...
									alertDiv("alert-danger",
											"This Testimonial has some data in system, so you can not delete this Offer!!");
								} else {
		...
		$('#addBannerForm').validate(
				{
					...
					messages : {
							...
							c_id : 'Please enter Campaign ID!',
								name : 'Please enter Campaign Name!'
