...
function uploadDocument(masterId,prefix){
		masterId = prefix + "_" + getRandomString() + "_" + getRandomString() + "_" + getRandomString() + "_" + getRandomString()+generatenumber() + "_" + masterId;
		var uploadDoc = new FormData();
		uploadDoc.append("file", docUploadBtn.files[0]);
		var text = $("#docUploadBtn").val();
		text = text.substring(text.lastIndexOf("\\") + 1, text.length);
		var updatedFile = masterId+"."+(text.substring(text.indexOf(".")+1,text.length)).trim();
