                    ...
                    <div id="page-content">
                        ...
                        <div class="content-header">
                            <div class="header-section">
                                <h1>
                                    <i class="fa fa-tasks"></i>Mechanics
                                </h1>
                            </div>
                        </div>
                            ...
                            <div class="col-sm-12 col-lg-12">
                               <div class="block full">
                                    ...
                                    <div class="block-title">
                                        <h2><i class="fa fa-tasks"></i><strong> Mechanics </strong> List</h2>
		     	...
		     	<div class="modal-content">
				    <!-- Add Contact Block -->
                                <div class="block" style="margin-bottom: 0px;padding-bottom: 0px;">
                                    <!-- Add Contact Title -->
                                    <div class="block-title">
                                        <h2><i class="fa fa-plus"></i> Add New Mechanics</h2>
                                    </div>
                                    <!-- END Add Contact Title -->

                                    <!-- Add Contact Content -->
                                    <form:form id="addMechanicForm" modelAttribute="addMechanic" method="POST" class="form-horizontal form-bordered">
                                        <div class="form-group">
                                            <label class="col-xs-3 control-label" for="mechanicname">Mechanic Name:</label>
                                            <div class="col-xs-9">
                                                <input type="text" name="mechanicname" id="mechanicname" class="form-control" placeholder="Enter mechanic name">
                                            </div>
                                        </div>
										<div class="form-group form-actions">
                                            <div class="col-xs-9 col-xs-offset-3" style="text-align: right;">
                                            	<input type="hidden" value="0" id="mechanic_id" name="mechanic_id">
...
$('#example-datatable').dataTable({
  ...
  function createNewCategory(id){
		...
		if(id!="0"){
		 $.ajax({
	   			type:"POST",
	   			url:"${pageContext.servletContext.contextPath}/masters/getmechanicdetails",
	   			data:$("#addMechanicForm").serialize()+"&mainCatId="+id.split("_")[1],
	   			dataType: 'json',
	   			async:false,
	   			success: function(response){
	   				$("#mechanicname").focus().val(response.mechanicname);
	   				$("#mechanic_id").val(response.id);
  ...
  function deleteCategory(id){
		...
		$.ajax({
			type:"GET",
			url:"${pageContext.servletContext.contextPath}/masters/deleteMechanic",
			...
			success: function(response){
				...
				if(response == "false"){
					alertDiv("alert-danger","This Mechanic has some data in the system, so you can not delete this mechanic!!");
				}else{
					$("#top-loading-stop").click();
					$("#mainCatTr_"+id.split("_")[1]).remove();
					alertDiv("alert-success","Mechanic deleted successfully!!");
				}
			}
		});
  ...
  function saveCategory(){
		...
		$('#addMechanicForm').validate().element("#mechanicname");
		if($("#mechanicname").val() != ""){
			$("#top-loading-start").click();
			$.ajax({
				type:"POST",
				url:"${pageContext.servletContext.contextPath}/masters/addMechanics",
				data:$("#addMechanicForm").serialize(),
				dataType: 'json',
				async:false,
				success: function(response){
				if(response.split("~")[0] == "y"){
						var msgText  = $("#mechanic_id").val() == 0 ? "redirect_success_common" : "redirect_success_common_update";
						$("#top-loading-stop").click();			
						window.location.href = "${pageContext.request.contextPath}/masters/clientdashboard/mechanicmaster/irLGex4aET8F+A2uPcRMvA==/1/"+msgText+"/0";
					}else{
  ...
  $('#addMechanicForm').validate({
	    ...
	    messages: {
	    	mechanicname: 'Please enter mechanic name !'
	    }
	});
